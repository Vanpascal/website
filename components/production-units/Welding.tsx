...
import { fetchWeldingProducts } from "@/app/actions/productActions";
import { fetchWeldingHOD } from "@/app/actions/staffActions";
...
function Welding() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        const data = await fetchWeldingProducts();
    ...
    const getHOD = async () => {
      try {
        const data = await fetchWeldingHOD();
        if (data.length > 0) {
